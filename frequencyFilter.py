...
def butter3d(low_cut: Union[float, None] = None,
             high_cut: Union[float, None] = None,
             order: int = 1,
             filter_path: str = None,
             use_existing: bool = True,
    ...
    filename = f"butter3d_order{order}"
    if low_cut is not None:
        filename += f"_lowcut_{low_cut}"
    if high_cut is not None:
        filename += f"_highcut_{high_cut}"
    ...
    if image is not None and len(image.shape) == 3:
        ...
        assert ydim is not None, f"y dimension not provided!"
        assert xdim is not None, f"x dimension not provided!"
        assert zdim is not None, f"z dimension not provided!"

    # Initialize arrays
    # -----------------

    high_cut_mask = np.ones((zdim, ydim, xdim), dtype=np.float64)
    low_cut_mask = np.ones_like(high_cut_mask)

    # find mid-point (must -0.5 to match pixel center position)
    z_mid, y_mid, x_mid = zdim / 2 - 0.5, ydim / 2 - 0.5, xdim / 2 - 0.5
    if verbose:
        print(f"Mid point of filter: y = {y_mid:.3f}, x = {x_mid:.3f}")

    grid = np.mgrid[0:zdim, 0:ydim, 0:xdim]  # grid[0] is z-coordinate, grid[1] is y-coordinate, , grid[2] is x-coordinate

    distance_to_mid = np.sqrt((grid[0] - z_mid) ** 2 + (grid[1] - y_mid) ** 2 + (grid[2] - x_mid) ** 2)

    if high_cut is not None:
        # no need to worry about dividing by 0 because we are not dividing by the distance-to-mid
        high_cut_mask = 1 / np.sqrt(
            1 + (np.sqrt((grid[0] - z_mid) ** 2 + (grid[1] - y_mid) ** 2 + (grid[2] - x_mid) ** 2) / high_cut) ** (2 * order))
